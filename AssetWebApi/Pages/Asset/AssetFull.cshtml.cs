...
{
    public class AssetFullModel : PageModel
    {
        public List<assetData> listAssetFull = new List<assetData>();
        public void OnGet()
        {
            try
            {
                ...
                {
                    conn.Open();
                    using (SqlCommand cmd = new SqlCommand("SELECT [Key Id],[N-Central ID],[CompanyName],[AssetName],[CurrentSyncUser],[LastSyncUser],[ContactID],[ContactName] FROM [Asset].[dbo].[AssetContact] WHERE [Filtered] = 0", conn))
                    {
